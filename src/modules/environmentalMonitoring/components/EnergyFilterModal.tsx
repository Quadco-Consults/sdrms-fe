...
interface EnergyFilterModalProps {
  open: boolean;
  onClose: () => void;
  filters: EnergyFilters;
  onApply: (filters: EnergyFilters) => void;
}

const emptyFilters: EnergyFilters = {
  energySource: "",
  scope: "",
  bu: "",
  renewableType: "",
...
export default function EnergyFilterModal({ open, onClose, filters, onApply }: EnergyFilterModalProps) {
  const [local, setLocal] = useState<EnergyFilters>(filters);

  if (!open) return null;

  const update = (key: keyof EnergyFilters, value: string) =>
      ...
      <div className="relative w-96 bg-white shadow-xl overflow-y-auto">
        <div className="p-6 space-y-5">
          ...
          <div className="space-y-1.5">
            <label className="text-sm text-gray-600">Energy Source</label>
            <select value={local.energySource} onChange={(e) => update("energySource", e.target.value)} className={selectClass}>
          ...
          <div className="space-y-1.5">
            ...
            <select value={local.scope} onChange={(e) => update("scope", e.target.value)} className={selectClass}>
              <option value="">choose any</option>
          ...
          {/* Renewable Type */}
          <div className="space-y-1.5">
            <label className="text-sm text-gray-600">Renewable Type</label>
            <select value={local.renewableType} onChange={(e) => update("renewableType", e.target.value)} className={selectClass}>
              <option value="">choose any</option>
