...
import { Upload, X, FileText, FileSpreadsheet, CheckCircle } from "lucide-react";
...
interface UploadWorkforceDataModalProps {
  open: boolean;
  onClose: () => void;
  onUpload: (files: File[]) => void;
}

export default function UploadWorkforceDataModal({
  open,
  onClose,
  onUpload,
}: UploadWorkforceDataModalProps) {
  ...
  const getFileIcon = (type: string) => {
    switch (type) {
      ...
      case "pdf":
        return <div className="w-10 h-10 rounded bg-red-500 text-white flex items-center justify-center text-xs font-bold">PDF</div>;
      case "xlsx":
      ...
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold">
            Upload Workforce Diversity Data
          </DialogTitle>
        </DialogHeader>
